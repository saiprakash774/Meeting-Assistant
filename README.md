...
Ten guardrails are now injected at the code level into every detail and chat prompt. They cannot be overridden by editing the prompts in Settings. The rules cover: answering the actual intent, staying on topic, using the transcript as the source of truth, not speculating, labelling uncertainty clearly, keeping every sentence relevant, using the injected date, never adding attribution signatures, never appending unsolicited offers to reformat or continue the output, and never inventing specific dates, times, names, or examples not present in the transcript.
...
**Deduplicated "do not repeat" titles sent to the model**

The suggestion system sends the titles from the previous 3 batches to the model as a "do not repeat" instruction. If the same suggestion title appeared in more than one of those batches, it was being sent to the model multiple times in the same instruction — for example, "Verify ABRN image uptime claim, Verify ABRN image uptime claim, ..." — which wastes tokens and can confuse the model.

The fix wraps the collected titles in a Set before building the instruction, so each title appears at most once regardless of how many batches it came from.

---

**Chat detail answers no longer invent specifics for thin context**

When a suggestion was expanded into a full answer in the chat panel, the model would sometimes fabricate concrete details — specific dates, times, names, or examples — in the Action Items section, even when the transcript contained no such information. For example, with only "Okay" responses in the transcript, the model would invent a follow-up meeting date like "April 22, 2026 at 10:00 AM ET".

A tenth code-level guardrail was added to the detail and chat system prompts: action items must be written in general terms when the transcript lacks specifics. The model may not use the session date to fabricate future appointment times or invent any concrete detail not explicitly present in the transcript.
